...
void EditorLayer::OnAttach() {
  ...
  TURT_CORE_INFO("attatching editor layer");

  // m_CheckerboardTexture =
  // AssetManager::CreateTexture("assets/textures/Checkerboard.png");

  FramebufferSpecification fbSpec;
  fbSpec.Width = 1280;
  fbSpec.Height = 720;
  m_Framebuffer = Framebuffer::Create(fbSpec);
  TURT_CORE_INFO("framebuffer created");
...
void EditorLayer::OnDetach() { TURT_PROFILE_FUNCTION(); }
...
void EditorLayer::OnEvent(Event &event) {
  ...
  dispatcher.Dispatch<KeyPressedEvent>(
      TURT_BIND_EVENT_FN(EditorLayer::OnKeyPressed));
}
...
bool EditorLayer::OnKeyPressed(KeyPressedEvent &event) {
  ...
  bool control = Input::IsKeyPressed(TURT_KEY_LEFT_CONTROL) ||
                 Input::IsKeyPressed(TURT_KEY_RIGHT_CONTROL);
  bool shift = Input::IsKeyPressed(TURT_KEY_LEFT_SHIFT) ||
               Input::IsKeyPressed(TURT_KEY_RIGHT_SHIFT);
...
void EditorLayer::OpenSceneFile() {
  std::string filepath =
      FileDialogs::OpenFile("Turtle Scene (*.turt)\0*.turt\0");
  if (!filepath.empty()) {
    m_ActiveScene = CreateRef<Scene>();
    m_ActiveScene->OnViewportResize((uint32_t)m_ViewportSize.x,
                                    (uint32_t)m_ViewportSize.y);
