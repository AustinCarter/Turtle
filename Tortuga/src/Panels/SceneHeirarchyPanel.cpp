...
void SceneHeirarchyPanel::DrawComponents(Entity entity) {
  ...
  // NOTE: Tag and Transform are currently being drawn here instead of in their
  // ui functions as all entities should have
  //        these components and it is desierable to ensure that they are always
  //        drawn first
  ...
  m_Context->m_Registry.visit(entity, [&](const auto component) {
    ...
    auto instance =
        type.template ctor<decltype(m_Context->m_Registry), entt::entity>()
            .invoke(std::ref(m_Context->m_Registry), (entt::entity)entity);
