...
struct AudioPlayerData {
  ...
  ma_device Device;
  // Probably don't want this to be fixed in the future so make variable to
  // facilitate possible future refactor
...
void AudioPlayer::Init() {
  TURT_CORE_INFO("initializing audio player");
  ...
  if (ma_device_init(NULL, &s_AudioData.DeviceConfig, &s_AudioData.Device) !=
      MA_SUCCESS) {
...
void AudioPlayer::Reset() { s_AudioData.Decoders.clear(); }

void AudioPlayer::Play(Ref<AudioDecoder> decoder) {
  s_AudioData.Decoders.emplace_back(decoder);
}

void AudioPlayer::Pause(Ref<AudioDecoder> decoder) { decoder->Pause(); }
...
void AudioPlayer::AudioPlayerDataCallback(ma_device *pDevice, void *pOutput,
                                          const void *pInput,
  ...
  while (decoder != s_AudioData.Decoders.end()) {
    ...
    if (decoder->get()->FinishedPlaying()) {
      ...
      uint32_t framesRead =
          decoder->get()->read_and_mix_s16((int16_t *)pOutput, frameCount);
      decoder++;
    }
  }
