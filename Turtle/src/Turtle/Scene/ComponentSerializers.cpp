...
void CameraComponent::Deserialize(YAML::Node &data, Entity entity) {
  ...
  comp.Camera.SetOrthographicNearClip(
      cameraProps["OrthographicNear"].as<float>());
  comp.Camera.SetOrthographicFarClip(
...
void ParticleSpawnerComponent::Deserialize(YAML::Node &data, Entity entity) {
  ...
  const auto &particleProps = data["Particle"];
  ...
  comp.Particle.VelocityVariation =
      particleProps["VelocityVariation"].as<glm::vec3>();
