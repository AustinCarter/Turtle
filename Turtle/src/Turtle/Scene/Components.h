...
// 	void Bind()
// 	{
// 		InstantiateScript = []() { return
// static_cast<ScriptableEntity*>(new T()); }; 		DestroyScript =
// [](NativeScriptComponent* nsc) {delete nsc->Instance; nsc->Instance =
// nullptr; }; 		Bound = true;
// 	}
// 	void Serialize(YAML::Emitter& out);
// 	void Deserialize(YAML::Node& out, Entity entity);
...
struct ParticleSpawnerComponent {
  ...
  ParticleSpawnerComponent() {
    ...
    Particle.SizeBegin = 0.3f, Particle.SizeVariation = 0.2f,
  ...
  ParticleSpawnerComponent(const ParticleProps &particle)
      : Particle(particle) {}
...
struct TileMapComponent {
  // NOTE: ON SERIALIZATION COULD STORE THE SIZE OF THE VECTORS TO ALLOW FOR
  // ONLY ALLOCATION DURING DESERIALIZATION
...
template <typename Component> static Component &Add(Entity entity) {
  if (entity.HasComponent<Component>())
    return entity.GetComponent<Component>();
  return entity.AddComponent<Component>();
}

// Register generic component information (Name, Get&Add constructor, Serialize,
// Deserialize, DrawUI )
template <typename Component>
static entt::meta_factory<Component> RegisterComponent(char const *name) {
  auto ret = entt::meta<Component>()
                 .type(entt::hashed_string{name})
                 .prop("Name"_hs, name)
                 .prop("Component"_hs)
                 .template ctor<>()
                 .template ctor<&Get<Component>>()
                 .template ctor<&Add<Component>>()
                 .template func<&Component::Serialize>("Serialize"_hs)
                 .template func<&Component::Deserialize>("Deserialize"_hs)
                 .template func<&Component::DrawUI>("DrawUI"_hs);

  std::string getterName = "Get__";
  getterName.append(name);

  char *getterName_c = (char *)malloc(128);
  strncpy(getterName_c, getterName.c_str(), 128);

  entt::meta<Entity>()
      .func<&Entity::GetComponent<Component>, entt::as_ref_t>(
          entt::hashed_string{getterName_c})
