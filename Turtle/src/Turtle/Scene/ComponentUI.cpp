...
// NOTE: Tag and Transform UI is being drawn in the SceneHeirarchy Panel at the
// moment to ensure they are shown at the top
...
void SpriteRendererComponent::DrawUI(Entity entity) {
  DrawComponent<SpriteRendererComponent>("Sprite", entity, [](auto &component) {
    ...
    if (component.Textured) {
      ImGui::Image((void *)component.RendererID, ImVec2{100, 100}, ImVec2{0, 1},
                   ImVec2{1, 0});
      ImGui::SameLine();
...
void ScriptComponent::DrawUI(Entity entity) {
  DrawComponent<ScriptComponent>("Script", entity, [](auto &component) {
    if (component.Script) {
      ...
      if (ImGui::Button(component.Script->GetFilepath().c_str(),
...
void AudioSourceComponent::DrawUI(Entity entity) {
  DrawComponent<AudioSourceComponent>(
      "Audio Source", entity, [](auto &component) {
        ...
        if (decoder) {
          ...
          // ImGui::SameLine();
          ImGui::PushFont(boldFont);
          ImGui::Button(decoder->GetFilepath().c_str(),
                        ImVec2{ImGui::GetContentRegionAvail().x, 0.0f});
          ImGui::PopFont();
...
void ParticleSpawnerComponent::DrawUI(Entity entity) {
  ...
  DrawComponent<ParticleSpawnerComponent>(
      "Particle Spawner", entity, [](auto &component) {
        ...
        ImGui::NextColumn();
        ImGui::PushItemWidth(ImGui::GetContentRegionAvailWidth() * 0.8f);
        ImGui::DragFloat("##Size Begin", &particle.SizeBegin, 0.01f, 0.0f,
                         1.0f);
        ImGui::PopItemWidth();
        ImGui::NextColumn();
        ...
        ImGui::NextColumn();
        ImGui::PushItemWidth(ImGui::GetContentRegionAvailWidth() * 0.8f);
        ImGui::DragFloat("##Size Variation", &particle.SizeVariation, 0.01f,
                         0.0f, 1.0f);
        ImGui::PopItemWidth();
        ImGui::NextColumn();
        ...
        ImGui::NextColumn();
        ImGui::PushItemWidth(ImGui::GetContentRegionAvailWidth() * 0.8f);
        ImGui::DragInt("##Emission Rate", (int *)(&component.EmissionRate),
                       0.03f, 0, 10);
        ImGui::PopItemWidth();
...
void TileSetComponent::DrawUI(Entity entity) {
  ...
  DrawComponent<TileSetComponent>("Tile Set", entity, [&](auto &component) {
    ...
    if (component.DisplayPallette) {
      ...
      ImGuiIO &io = ImGui::GetIO();
      ImGui::BeginChild("Tile Set",
                        ImVec2{ImGui::GetContentRegionAvail().x,
                               ImGui::GetContentRegionAvail().y - 30},
                        false, ImGuiWindowFlags_HorizontalScrollbar);
      ImVec2 p = ImGui::GetCursorScreenPos();
      ImGui::Image((void *)component.TileSet->GetRendererID(),
                   ImVec2{(float)component.TileSet->GetWidth(),
                          (float)component.TileSet->GetHeight()},
                   ImVec2{0, 1}, ImVec2{1, 0});

      if (!component.TileWidth)
        component.TileWidth = component.TileSet->GetWidth();
      if (!component.TileHeight)
        component.TileHeight = component.TileSet->GetHeight();

      for (int x = 1; x < component.TileSet->GetWidth() / component.TileWidth;
           x++) {
        ImGui::GetWindowDrawList()->AddLine(
            ImVec2(p.x + component.TileWidth * x, p.y),
            ImVec2(p.x + component.TileWidth * x,
                   p.y + component.TileSet->GetHeight()),
            IM_COL32(0, 0, 0, 255), 1.0f);
      }
      for (int y = 1; y < component.TileSet->GetHeight() / component.TileHeight;
           y++) {
        ImGui::GetWindowDrawList()->AddLine(
            ImVec2(p.x, p.y + component.TileHeight * y),
            ImVec2(p.x + component.TileSet->GetWidth(),
                   p.y + component.TileHeight * y),
            IM_COL32(0, 0, 0, 255), 1.0f);
      }
      ...
      if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(0)) {
        ...
        // TURT_WARN("selected: {0}, {1}", (int)(io.MousePos.x - p.x)/128,
        // (int)(io.MousePos.y - p.y)/128);
      }
      ...
      ImGui::GetWindowDrawList()->AddLine(
          ...
          ImVec2(p.x + xPos + component.TileWidth, p.y + yPos),
          IM_COL32(255, 0, 0, 255), 3.0f);
      ImGui::GetWindowDrawList()->AddLine(
          ImVec2(p.x + xPos + component.TileWidth, p.y + yPos),
          ImVec2(p.x + xPos + component.TileWidth,
                 p.y + yPos + component.TileHeight),
          IM_COL32(255, 0, 0, 255), 3.0f);
      ImGui::GetWindowDrawList()->AddLine(
          ImVec2(p.x + xPos, p.y + yPos),
          ImVec2(p.x + xPos, p.y + yPos + component.TileHeight),
          IM_COL32(255, 0, 0, 255), 3.0f);
      ImGui::GetWindowDrawList()->AddLine(
          ImVec2(p.x + xPos, p.y + yPos + component.TileHeight),
          ImVec2(p.x + xPos + component.TileWidth,
      ...
      ImVec2 min =
          ImVec2((float)(SelectedX * component.TileWidth) / TileSetWidth,
                 1.0f - (float)(component.SelectedY * component.TileHeight) /
                            TileSetHeight);
      ImVec2 max = ImVec2(
          (float)((SelectedX + 1) * component.TileWidth) / TileSetWidth,
          1.0f - (float)((component.SelectedY + 1) * component.TileHeight) /
                     TileSetHeight);

      ImGui::Image(
          (void *)component.TileSet->GetRendererID(),
          ImVec2{(float)component.TileWidth, (float)component.TileHeight}, min,
          max);
    }
