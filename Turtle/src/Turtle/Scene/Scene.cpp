...
Scene::Scene() {
  m_Registry.on_construct<CameraComponent>().connect<&Scene::OnCameraConstruct>(
      *this);
  m_Registry.on_construct<TileSetComponent>().connect<&OnTileSetConstruct>();
}

Scene::~Scene() { AudioPlayer::Reset(); }
...
void Scene::OnUpdate(Timestep ts) {
  ...
  if (primaryCamera) {
    ...
    {
      auto view =
          m_Registry.view<TransformComponent, ParticleSpawnerComponent>();

      for (auto entity : view) {
        auto [transform, spawner] =
            view.get<TransformComponent, ParticleSpawnerComponent>(entity);
    ...
    //   for (auto entity : view) {
    //     auto [grid, tileMap] =
    //         view.get<GridComponent, TileMapComponent>(entity);
    //
    //     int index = 0;
    //     for (auto &pos : tileMap.Positions) {
    //       Renderer2D::DrawQuad(glm::vec2(pos.x, pos.y),
    //       glm::vec2(1.0f, 1.0f),
    //                            tileMap.Textures[index], tileMap.Tint);
    //       index++;
    //     }
    //
    //     if (grid.Active && grid.GridSize > 0) {
    //       float orthoSize =
    //           ((SceneCamera *)primaryCamera)->GetOrthographicSize();
    //       float aspectRatio = ((SceneCamera
    //       *)primaryCamera)->GetAspectRatio();
    //
    ...
    //                 ((x * grid.GridSize) +
    //                  (cameraTransform[3][0] -
    //                   fmod(cameraTransform[3][0], grid.GridSize)) -
    //                  (((orthoSize * aspectRatio) / 2) -
    //                   fmod((orthoSize * aspectRatio) / 2, grid.GridSize))),
    //                 cameraTransform[3][1]),
    //             glm::vec2(.001f * orthoSize, orthoSize),
    ...
    //                 (y * grid.GridSize) +
    //                     (cameraTransform[3][1] -
    //                      fmod(cameraTransform[3][1], grid.GridSize)) -
    //                     ((orthoSize / 2) - fmod(orthoSize / 2,
    //                     grid.GridSize))),
    //             glm::vec2(orthoSize * aspectRatio, .0015f * orthoSize),
    //             glm::vec4(0.5f, 0.5f, 0.5f, 1.0f));
    //       }
    //
    //       break;
...
void Scene::SerializeScene(const std::string &filepath) {
  ...
  m_Registry.each([&](auto entityID) {
    ...
    m_Registry.visit(entityID, [&](const auto component) {
      ...
      auto instance =
          type.template ctor<decltype(m_Registry), entt::entity>().invoke(
              std::ref(m_Registry), entityID);
    ...
    for (YAML::Node entity : entities) {
      ...
      for (auto component : entity) {
        ...
        if (type && type.prop("Component"_hs)) {
          ...
          type.func("Deserialize"_hs)
              .invoke(instance, std::ref(component.second), deserializedEntity);
        }
      }
    }
  }
