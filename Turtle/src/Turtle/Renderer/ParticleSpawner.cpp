...
ParticleSpawner::ParticleSpawner() { m_ParticlePool.resize(1000); }
...
void ParticleSpawner::OnRender() {
  ...
  for (auto &particle : m_ParticlePool) {
    ...
    // PARTICLE GENERATOR WILL ALWAYS RESULT IN A FULL DRAW CALL
    Renderer2D::DrawRotatedQuad(particle.Position, glm::vec2{size, size},
...
void ParticleSpawner::Emit(const ParticleProps &particleProps) {
  ...
  particle.Rotation = particleProps.RandomRotate
  ...
  particle.Velocity.z +=
      particleProps.VelocityVariation.z * (Random::Float() - 0.5f);
