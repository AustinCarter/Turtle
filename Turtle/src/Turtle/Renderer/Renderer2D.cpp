...
struct Renderer2DData {
  ...
  static const uint32_t MaxTextureSlots =
      32; // TODO: Render capabilities - poll gpu for texture slots
...
void Renderer2D::Init() {
  ...
  TURT_CORE_INFO("\t\tcreating vertex array");
  s_RenderData.QuadVertexArray = VertexArray::Create();

  TURT_CORE_INFO("\t\tcreating vertex buffers");
  s_RenderData.QuadVertexBuffer =
      VertexBuffer::Create(s_RenderData.MaxVertices * sizeof(QuadVertex));

  TURT_CORE_INFO("\t\tSetting buffer layout");
  ...
  TURT_CORE_INFO("\t\tcreating quadIndicies");
  ...
  Ref<IndexBuffer> quadIB =
      IndexBuffer::Create(quadIndicies, s_RenderData.MaxIndices);
  s_RenderData.QuadVertexArray->SetIndexBuffer(quadIB);
  TURT_CORE_INFO("\t\tdeleting quad indicies");
  // WARNING: TODO: ASSUMES THAT DATA IS IMMEDIATLY UPLOADED, COULD CAUSE DATA
  // DELETION -> CRASH
  delete[] quadIndicies;

  TURT_CORE_INFO("\t\tCreating textures");
  ...
  TURT_CORE_INFO("\t\tcreating shader");
  s_RenderData.TextureShader =
      Shader::Create("Turtle/assets/shaders/Texture.glsl");
  s_RenderData.TextureShader->Bind();
  s_RenderData.TextureShader->SetIntArray("u_Textures", samplers,
                                          s_RenderData.MaxTextureSlots);
...
void Renderer2D::BeginScene(const OrthographicCamera &camera) {
  ...
  s_RenderData.TextureShader->Bind();
  s_RenderData.TextureShader->SetMat4("u_ViewProjection",
...
void Renderer2D::EndScene() {
  ...
  uint32_t dataSize = (uint32_t)((uint8_t *)s_RenderData.QuadVertexBufferPtr -
                                 (uint8_t *)s_RenderData.QuadVertexBufferBase);
  s_RenderData.QuadVertexBuffer->SetData(s_RenderData.QuadVertexBufferBase,
                                         dataSize);
  Flush();
}
...
void Renderer2D::DrawQuad(const glm::mat4 &transform, const glm::vec4 &color) {
  ...
  for (size_t i = 0; i < quadVertexCount; i++) {
    s_RenderData.QuadVertexBufferPtr->Position =
        transform * s_RenderData.QuadVertexPositions[i];
...
Renderer2D::Statistics Renderer2D::GetStats() { return s_RenderData.Stats; }
